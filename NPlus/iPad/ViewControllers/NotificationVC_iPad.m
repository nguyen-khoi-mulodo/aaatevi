...
@interface NotificationVC_iPad ()

@end

@implementation NotificationVC_iPad
...
//- (void) loadListChannelLikedWithPageIndex:(int)pageIndex pageSize:(int)pageSize{
//    if (self.dataSources.count == 0) {
//        [self showConnectionErrorView:NO];
//        [self showNoDataView:NO];
//        [self showLoadingDataView:YES];
...
- (UIGridViewCell *) gridView:(UIGridView *)grid cellForRowAt:(int)rowIndex AndColumnAt:(int)columnIndex andSection:(int) section
{
    ...
    static NSString *CellIdentifier1 = @"notificationItemCell";
    ...
    if (arr.count > index) {
        NotificationItemCell *cell = (NotificationItemCell *)[grid dequeueReusableCell:CellIdentifier1];
        if (cell == nil) {
            cell = [[NotificationItemCell alloc] init];
        }
        [cell setDelegate:self];
        id item = [arr objectAtIndex:index];
        if ([item isKindOfClass:[LocalNotif class]]) {
            LocalNotif* noti = (LocalNotif*)item;
            [cell setContent:noti];
        }
        return cell;
    }
    return nil;
