...
- (void)viewDidLoad {
    ...
    [self.tbMain registerNib:[UINib nibWithNibName:@"DownloadingItemCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"downloadingItemCellIdef"];
...
- (void) showHeader:(BOOL) isShow{
    if (isShow) {
        ...
        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_SIZE.width, headerView.frame.size.height + lineView.frame.size.height)];
...
- (void) setContentHeader{
    if (isStoppingAll) {
        [btnHeader setImage:[UIImage imageNamed:@"icon-tieptuc-v2"] forState:UIControlStateNormal];
        [lbHeader setText:@"Tải tất cả"];
    }else{
        [btnHeader setImage:[UIImage imageNamed:@"icon-tamdung-v2"] forState:UIControlStateNormal];
        [lbHeader setText:@"Tạm ngưng tất cả"];
    }
}
...
- (void) loadData{
    ...
    NSArray* reversedArray = [[arrDownloading reverseObjectEnumerator] allObjects];
    ...
    if (arrDownloading.count > 0) {
        ...
        isStoppingAll = [self checkStopAll];
...
- (BOOL) checkStopAll{
    ...
    for (FileDownloadInfo* info in self.dataSources) {
        if (info.isDownloading || info.isWaiting) {
            isStopAll = NO;
            return isStopAll;
        }
    }
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *itemCellIdef = @"downloadingItemCellIdef";
    DownloadingItemCell *cell = (DownloadingItemCell*)[self.tbMain dequeueReusableCellWithIdentifier:itemCellIdef];
    if (!cell) {
        cell = [Utilities loadView:[DownloadingItemCell class] FromNib:@"DownloadingItemCell"];
    }
    ...
    if (tableView.editing) {
        ...
        if (self.dataSources.count > indexPath.row) {
            ...
            if ([info.status isEqualToString:@"0"]) {
                ...
                [[DownloadManager sharedInstance] pause:info];
                NSLog(@"Pause with task:%d", (int) info.taskIdentifier);
            }else{
                [[DownloadManager sharedInstance] resume:info];
                NSLog(@"Resume with task:%d", (int) info.taskIdentifier);
            }
            isStoppingAll = [self checkStopAll];
            [self setContentHeader];
        }
...
- (IBAction) pauseAll:(id) sender{
    ...
    [self setContentHeader];
    if (isStoppingAll) {
        [[DownloadManager sharedInstance] pauseList:self.dataSources];
    }else{
        for (FileDownloadInfo* info in self.dataSources) {
            [[DownloadManager sharedInstance] resume:info];
        }
...
-(void)downloadManager:(DownloadManager *)downloadManager updateDownload:(FileDownloadInfo *)downloadInfo{
    ...
    {
        if([obj isKindOfClass:[DownloadingItemCell class]])
        {
            DownloadingItemCell *cell=(DownloadingItemCell *)obj;
            [cell updateCell:downloadInfo.taskIdentifier andInfo:downloadInfo];
        }
    }
    NSLog(@"Updating download at DownloadingController with task:%d", (int)downloadInfo.taskIdentifier);
}
