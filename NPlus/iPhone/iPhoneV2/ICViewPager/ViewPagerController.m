...
#define kTabWidth (IS_IPAD ? 225.0f : 128.0f)
...
#define kTabsViewBackgroundColor [UIColor colorWithRed:7.0/255.0 green:60.0/255.0 blue:110.0/255.0 alpha:1.0]
...
- (void)layoutSubviews {
    ...
    if (!IS_IPAD) {
        if (_isScrollHeader) {
            topLayoutGuide = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabOffset withDefault:0.0];
        }
    }else{
        if (self.tabCount == 2) {
            topLayoutGuide = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabOffset withDefault:0.0f];
        }
    }
    ...
    if (_isScrollBottomBar) {
        //        height = CGRectGetHeight(self.view.frame)+50;
    }
    frame.size.height = height - (topLayoutGuide + CGRectGetHeight(self.tabsView.frame)) - (self.tabBarController.tabBar.hidden ? 0 : CGRectGetHeight(self.tabBarController.tabBar.frame));
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    [activeTabView setAlpha:0.5f];
    for (UILabel *lbl in activeTabView.subviews) {
        lbl.font = [UIFont fontWithName:kFontLight size:15.0f];
//        lbl.textColor = RGBA(255, 255, 255, 0.3);
        lbl.textColor = UIColorFromRGB(0x212121);
        lbl.adjustsFontSizeToFitWidth = YES;
    }
    // Set to-be-active tab selected
    activeTabView = [self tabViewAtIndex:activeTabIndex];
    activeTabView.selected = YES;
    [activeTabView setAlpha:1.0f];
    for (UILabel *lbl in activeTabView.subviews) {
        lbl.font = [UIFont fontWithName:kFontLight size:15.0f];
        lbl.textColor = UIColorFromRGB(0x00adef);
        lbl.adjustsFontSizeToFitWidth = YES;
    }
...
- (void)defaultSetup {
    ...
    if (!IS_IPAD) {
        contentSizeWidth = (SCREEN_WIDTH - self.tabCount * [self.delegate viewPager:self valueForOption:ViewPagerOptionTabWidth withDefault:96.0])/2;
    }else{
        contentSizeWidth = (self.view.frame.size.width - self.tabCount * [self.delegate viewPager:self valueForOption:ViewPagerOptionTabWidth withDefault:kTabWidth])/2;
    }
    // Give the standard offset if fixFormerTabsPositions is provided as YES
    if ([self.fixFormerTabsPositions boolValue]) {
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++) {
        ...
        CGRect frame = tabView.frame;
        frame.origin.x = contentSizeWidth;
        if (self.tabCount == 4 && [self.tabWidth floatValue] == 80.0f) {
            if (i == 1) {
                frame.size.width = [self.tabWidth floatValue] + 6.0f;
            }else{
                frame.size.width = [self.tabWidth floatValue];
            }
        }else{
