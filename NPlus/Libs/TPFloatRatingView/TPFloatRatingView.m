...
- (void)layoutSubviews
{
    ...
    CGSize imageViewSize = [self sizeForImage:self.emptySelectedImage inSize:CGSizeMake(width, height)];
    CGFloat dX = 8;
    CGFloat Y = 94;
    CGFloat X = 78;
    for (int i = 0; i < self.emptyImageViews.count; ++i) {
        CGRect imageFrame = CGRectMake(X + i * (dX + imageViewSize.width), Y, imageViewSize.width, imageViewSize.height);
