...
- (UICollectionViewLayoutAttributes *)getAttributesForPage:(NSUInteger)page atOffset:(CGPoint)offset pageWidth:(CGSize)pageSize index:(NSUInteger)index progress:(CGFloat)progress element:(ESConveyorElement *)element
{
    ...
    if ([effects containsObject:@(ESConveyorEffectScalex1)])
    {
        attr.transform = CGAffineTransformMakeScale(progress, progress);
    }
    if ([effects containsObject:@(ESConveyorEffectRotation0)])
    {
        attr.transform = CGAffineTransformMakeRotation(DegreesToRadians(45 * progress));
    }
    if ([effects containsObject:@(ESConveyorEffectRotation45)])
    {
        attr.transform = CGAffineTransformMakeRotation(DegreesToRadians(45 * progress));
    }
